...
function ErrorBox({ message, requiresUpgrade, requiresAuth }) {
  if (requiresUpgrade) {
    return (
      <div className={styles.upgradeWall}>
        <div className={styles.upgradeIcon}>🔒</div>
        <h3>Pro-Feature</h3>
        <p>Diese Funktion ist nur für Pro-User verfügbar.</p>
        <a href="/pricing" className={styles.upgradeBtn}>Jetzt upgraden – CHF 19/Monat</a>
      </div>
    )
  }
  if (requiresAuth) {
    return (
      <div className={styles.upgradeWall}>
        ...
        <p>Bitte registriere dich um diese Funktion zu nutzen.</p>
        <a href="/auth" className={styles.upgradeBtn}>Kostenlos registrieren</a>
      </div>
    )
  }
...
function AnalyseTab({ user }) {
  ...
  return (
    <div>
      ...
      {error && <ErrorBox message={error?.message} requiresUpgrade={error?.requiresUpgrade} requiresAuth={error?.requiresAuth} />}
      {result && (
...
function CrawlTab() {
  ...
  async function handleSubmit(e) {
    e.preventDefault()
    if (!url.trim()) return
    setLoading(true); setResult(null); setError(null); setFilter('all'); setTypeFilter('all')
    try {
    ...
    <div>
      <form className={styles.searchBar} onSubmit={handleSubmit}>
        <input type="text" className={styles.urlInput} placeholder="https://deine-website.ch"
          value={url} onChange={e => setUrl(e.target.value)} />
        <button type="submit" className={styles.btnPrimary} disabled={loading}>
          {loading ? 'Crawle…' : 'Deep Crawl starten'}
        </button>
      </form>
      {loading && (
        <div className={styles.loadingWrap}>
          <div className={styles.loadingBar}><div className={styles.loadingFill} /></div>
          <p className={styles.loadingText}>Deep Crawl läuft – alle Seiten, Links, CTAs werden geprüft…<br/>
          <span style={{fontSize:12, color:'#aaa'}}>Kann je nach Website-Grösse 30–60 Sekunden dauern.</span></p>
        </div>
      )}
      {error && <ErrorBox message={error?.message} requiresUpgrade={error?.requiresUpgrade} requiresAuth={error?.requiresAuth} />}
      ...
      {result && (
        <div className={styles.resultsWrap}>
          <div className={styles.infoBox} style={{background: result.summary.urlSource === 'sitemap' ? '#E1F5EE' : '#E6F1FB', color: result.summary.urlSource === 'sitemap' ? '#0F6E56' : '#185FA5'}}>
            {result.summary.urlSource === 'sitemap'
              ? `✓ Sitemap gefunden (${result.summary.sitemapSource}) – ${result.summary.totalFound} URLs entdeckt`
              : `ℹ Keine Sitemap gefunden – URLs via HTML-Crawling entdeckt (${result.summary.totalFound} gefunden)`
            }
            {result.summary.truncated && ` · Erste ${result.summary.total} werden geprüft`}
          </div>

          {/* Zusammenfassung */}
          <div className={styles.metricsGrid} style={{ gridTemplateColumns: 'repeat(3, minmax(0,1fr))' }}>
            {[
              { label: 'Seiten gecrawlt', value: result.summary.pagesVisited, color: '#1a1a1a' },
              { label: 'URLs geprüft total', value: result.summary.total, color: '#1a1a1a' },
              { label: '✓ Erreichbar', value: result.summary.ok, color: '#1D9E75' },
              { label: '✕ Nicht gefunden (404)', value: result.summary.notFound, color: result.summary.notFound > 0 ? '#E24B4A' : '#888' },
              { label: '✕ Server-Fehler (5xx)', value: result.summary.errors, color: result.summary.errors > 0 ? '#E24B4A' : '#888' },
              { label: '↪ Weiterleitungen', value: result.summary.redirects, color: result.summary.redirects > 0 ? '#185FA5' : '#888' },
            ].map((m, i) => (
              <div key={i} className={styles.metricCard}>
                <span className={styles.mLabel}>{m.label}</span>
          ...
          <div className={styles.crawlSummaryBar} style={{ borderColor: hasProblems ? '#E24B4A' : '#1D9E75', background: hasProblems ? '#FCEBEB' : '#E1F5EE' }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 12, color: '#888', marginTop: 3 }}>
                {result.summary.pagesVisited} Seiten gecrawlt · {result.summary.externalChecked} externe Links geprüft
              </p>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
            <span style={{ fontSize: 12, color: '#888', alignSelf: 'center' }}>Typ:</span>
            {[
              { key: 'all', label: 'Alle' },
              { key: 'internal', label: `Intern (${result.urls.filter(u=>u.type==='internal').length})` },
              { key: 'external', label: `Extern (${result.urls.filter(u=>u.type==='external').length})` },
            ].map(f => (
              <button key={f.key}
                className={`${styles.filterBtn} ${typeFilter === f.key ? styles.filterBtnActive : ''}`}
                onClick={() => setTypeFilter(f.key)}>{f.label}</button>
            ))}
          </div>

          <div className={styles.filterBar}>
            <span style={{ fontSize: 12, color: '#888', alignSelf: 'center' }}>Status:</span>
            {[
                      ...
                      <tr key={i}>
                        <td className={styles.urlCell}>
                          <a href={u.url} target="_blank" rel="noopener">
                            {u.type === 'internal' ? (u.url.replace(result.domain, '') || '/') : u.url}
                          </a>
                        </td>
                        <td>
                          <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 99, fontWeight: 500, background: u.type === 'internal' ? '#E6F1FB' : '#F3F3F1', color: u.type === 'internal' ? '#185FA5' : '#666' }}>
                            {u.type === 'internal' ? 'Intern' : 'Extern'}
                          </span>
                        </td>
                        <td><StatusBadge category={u.category} status={u.status} /></td>
                        <td style={{ fontSize: 12, color: '#888' }}>
                          {u.linksFound > 0 ? u.linksFound : '–'}
                        </td>
                        <td className={styles.kwCell} style={{ fontSize: 12, color: '#666' }}>
        ...
        <div className={styles.navRight}>
          {user ? (
            <>
              ...
              <a href="/pricing" className={styles.navBtn}>Upgrade</a>
            ...
            <>
              <a href="/pricing" className={styles.navBtn}>Preise</a>
